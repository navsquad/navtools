...
def eci2ecefDcm(dt: float) -> np.ndarray:
    ...
    # Groves 2.145
    C_i_e = np.array(
        [[cos_wie, sin_wie, 0.0], [-sin_wie, cos_wie, 0.0], [0.0, 0.0, 1.0]],
        dtype=np.double,
    )
    return C_i_e
...
def ned2eciDcm(lla: np.ndarray, dt: np.ndarray) -> np.ndarray:
    ...
    C_n_i = np.array(
        [
            [-sinPhi * cos_lam_wie, -sin_lam_wie, -cosPhi * cos_lam_wie],
...
def ned2ecefDcm(lla: np.ndarray) -> np.ndarray:
    ...
    C_n_e = np.array(
        [
            [-sinPhi * cosLam, -sinLam, -cosPhi * cosLam],
...
def enu2eciDcm(lla: np.ndarray, dt: np.ndarray) -> np.ndarray:
    ...
    C_n_i = np.array(
        [
            [-sin_lam_wie, -sinPhi * cos_lam_wie, cosPhi * cos_lam_wie],
...
def enu2ecefDcm(lla: np.ndarray) -> np.ndarray:
    ...
    C_n_e = np.array(
        [
            [-sinLam, -cosLam * sinPhi, cosLam * cosPhi],
...
# === NED2ECEFW ===
@njit(cache=True, fastmath=True)
def ned2ecefw(w_eb_e: np.ndarray, lla0: np.ndarray) -> np.ndarray:
...
def ned2ecia(
    ...
    OMEGA_ie_n = skew(
        np.array(
            [
                GNSS_OMEGA_EARTH * np.cos(lla0[0]),
                0.0,
                -GNSS_OMEGA_EARTH * np.sin(lla0[0]),
            ]
        )
    )
    C_n_i = ned2eciDcm(lla0, dt)
